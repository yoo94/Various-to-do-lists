...
const $numRemindedTodos = $("#num-reminded-todos")
...
function setFilter(filter) {
    $(todoList).attr("data-filter", filter)
}

$("#btn-show-all").click(() => setFilter("all"))
$("#btn-show-active").click(() => setFilter("active"))
$("#btn-show-completed").click(() => setFilter("completed"))


function toggleTodoCompletion($todo, completed) {
    $todo.toggleClass("completed", completed)
    $todo.find("input[type='checkbox']").prop("checked", completed)
}
...
$("#btn-toggle-all").click(event => {
    ...
    $todoList.find(".todo").each((index, todo) => {
        toggleTodoCompletion($(todo), !isAllChecked)
    })
})
